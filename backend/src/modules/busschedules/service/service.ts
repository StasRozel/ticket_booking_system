...
import { urgentCallRepository } from "../../urgentcalls/repository/repository";
...
export class BusScheduleService {
  ...
  async createBusSchedule(
    data: Partial<BusSchedule>
  ): Promise<{ success: boolean; data?: BusSchedule; error?: string }> {
    ...
    try {
      ...
      return {
        success: false,
        error: "Ошибка при создании расписания автобуса",
      };
    }
  }
  ...
  async updateBusSchedule(
    ...
    try {
      ...
      if (!existingSchedule) {
        return {
          success: false,
          error: `Расписание автобуса с ID ${id} не найдено`,
        };
      }
      ...
      if (!busSchedule) {
        return {
          success: false,
          error: "Не удалось обновить расписание автобуса",
        };
      }
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Ошибка при обновлении расписания автобуса",
      };
    }
  }
  ...
  async replaceDriverAndBus(
    ...
    try {      
      const driver = await driverRepository.findOneById(driver_id as number);
      ...
      const updated = await busScheduleRepository.replaceDriverAndBus(
        id,
        driver.bus_id
      );
      ...
      await urgentCallRepository.update(unrgent_call_id, { driverId: driver_id, accepted: true });
